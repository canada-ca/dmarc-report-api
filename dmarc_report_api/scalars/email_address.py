...
class EmailAddress(Scalar):
    ...
    def serialize(value):
        ...
        if not EMAIL_ADDRESS_REGEX.search(value.lower()):
            raise GraphQLError(
                "Value is not a valid email address : " + str(value.lower())
            )
    ...
    def parse_literal(node):
        if not isinstance(node, ast.StringValue):
            raise GraphQLError(
                "Can only validate strings as email address's but got a : "
                + str(type(node))
            )

        if not EMAIL_ADDRESS_REGEX.search(node.value.lower()):
            raise GraphQLError(
                "Value is not a valid email address : " + str(node.value.lower())
            )
